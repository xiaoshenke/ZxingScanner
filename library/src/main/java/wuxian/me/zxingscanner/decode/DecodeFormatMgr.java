...
public final class DecodeFormatMgr {
    ...
    private DecodeFormatMgr() {
    }

    static Vector<BarcodeFormat> parseDecodeFormats(Intent intent) {
        List<String> scanFormats = null;
        String scanFormatsString = intent
                .getStringExtra(DecodeConstants.Scan.SCAN_FORMATS);
        if (scanFormatsString != null) {
            scanFormats = Arrays.asList(COMMA_PATTERN.split(scanFormatsString));
        }
        return parseDecodeFormats(scanFormats,
                intent.getStringExtra(DecodeConstants.Scan.MODE));
    }

    static Vector<BarcodeFormat> parseDecodeFormats(Uri inputUri) {
        List<String> formats = inputUri
                .getQueryParameters(DecodeConstants.Scan.SCAN_FORMATS);
        if (formats != null && formats.size() == 1 && formats.get(0) != null) {
            formats = Arrays.asList(COMMA_PATTERN.split(formats.get(0)));
        }
        return parseDecodeFormats(formats,
                inputUri.getQueryParameter(DecodeConstants.Scan.MODE));
    }
    ...
    private static Vector<BarcodeFormat> parseDecodeFormats(
        ...
        if (decodeMode != null) {
            if (DecodeConstants.Scan.PRODUCT_MODE.equals(decodeMode)) {
                return PRODUCT_FORMATS;
            }
            if (DecodeConstants.Scan.QR_CODE_MODE.equals(decodeMode)) {
                return QR_CODE_FORMATS;
            }
            if (DecodeConstants.Scan.DATA_MATRIX_MODE.equals(decodeMode)) {
                return DATA_MATRIX_FORMATS;
            }
            if (DecodeConstants.Scan.ONE_D_MODE.equals(decodeMode)) {
